...
import { acceptBooking } from "@/features/bookings/services/bookingService";
import { Role } from "@/generated/prisma";

/**
 * PATCH /api/bookings/[id]/accept - Accept a booking request
 * Only the vehicle owner can accept bookings
 */
  ...
  try {
    ...
    if (session.user.role !== Role.OWNER && session.user.role !== Role.ADMIN) {
      return NextResponse.json(
        { error: "Only vehicle owners can accept bookings" },
        { status: 403 },
      );
    }
